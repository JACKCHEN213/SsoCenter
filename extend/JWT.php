...
class JWT
{
    ...
    public static function decode(string $jwt, $key, array $allowed_algs = array()): object
    {
        ...
        // Check if the nbf it is defined. This is the time that the
        // token can actually be used. If it's not yet that time, abort.
        if (isset($payload->nbf) && $payload->nbf > ($timestamp + static::$leeway)) {
            throw new Exception(
                'Cannot handle token prior to ' . date('Y-m-d\TH:i:sO', $payload->nbf)
            );
        }
        ...
        if (isset($payload->iat) && $payload->iat > ($timestamp + static::$leeway)) {
            throw new Exception(
                'Cannot handle token prior to ' . date('Y-m-d\TH:i:sO', $payload->iat)
            );
        }
     ...
     */
    public static function encode($payload, string $key, string $alg = 'HS256', $keyId = null, $head = null): string
    {
    ...
    public static function sign(string $msg, string $key, string $alg = 'HS256'): string
    {
        ...
        throw new Exception("Sign drive not exist: $function");
    }
